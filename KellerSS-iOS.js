...
function parseIpsFile(content) {
    try {
        let trimmed = content.trim()
            ...
            let header = null
            try { header = headerLine ? JSON.parse(headerLine) : null }
        catch (e) {}
        let entries = []
            try { entries = dataLine ? JSON.parse(dataLine) : [] }
        catch (e) {}
...
function buildHTML(findings, netEntries, cheatAppFindings, knownCheatFindings, ipsFindings, ipsMeta, replaceFindings, ghostAppFindings, proxyLoginFindings, filename) {
    ...
    let uptimeStr = "?"
        ...
        if (firstTs && lastTs) {
            let diffMs = lastTs - firstTs
                ...
                if (diffD > 0)      uptimeStr = `${diffD
        }d ${ remH }h ${ remMin }min`
        else if (diffH > 0) uptimeStr = `${diffH
}h ${ remMin }min`
  ...
  for (let f of replaceFindings) {
      let label = f.isProxyApp
          ...
          ? `${f.appName
  } — ativo na rede mas ausente no app_usage.Possível IPA falso instalado via sideload e deletado.`
  ...
  let ffSessionRows = ffSessions.map((s, i) = > {
      ...
      .domain - badge.medium{ background:#2a2000; color:#ffbb00; }
          ...
          <div class = "hero-credits">por <span class = "credit-name">Maokol< / span> &middot; <span class = "credit-name">nloz< / span> &middot; <span class = "credit-name">Luzxin< / span>< / div>
          <div class = "lang-bar">
...
function buildLangScript() {
    return `<script >
        var TRANSLATIONS = {
          ...
          es: {
            eyebrow: "Scanner iOS",
            credits : "por MAIKOL · NLOZ · Luxzin",
            fileLabel : "Archivo:",
            start : "Inicio",
            lastRecord : "Último registro",
...
async function readFile(path) {
    ...
    if (!content) {
        try { content = FileManager.local().readString(path) }
        catch (e2) {}
    }
    return content
}
